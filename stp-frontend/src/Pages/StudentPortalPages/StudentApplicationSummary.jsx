...
const StudentApplicationSummary = ({}) => {
  ...
  const generateEnhancedPDF = async () => {
    ...
    try {
      ...
      yOffset = addText("Application Summary", 105, yOffset, {
        align: "center",
      });
      yOffset += 10;
      ...
      if (courseInfo?.logo) {
        try {
          const img = new Image();
          img.src = `${import.meta.env.VITE_BASE_URL}storage/${
            courseInfo.logo
          }`;
      ...
      const applicantInfo = [
        ...
        `Contact Number: ${basicInfo?.country_code || ""} ${
          basicInfo?.contact || ""
        }`,
      ...
      const courseInfoData = [
        ...
        `Estimate Fee: RM ${
          courseInfo?.cost?.toLocaleString() || "0.00"
        } / year`,
      ];
        ...
        if (category.id !== 32) {
          ...
          if (cgpaInfo.cgpa !== null || cgpaInfo.programName) {
            ...
            if (cgpaInfo.programName) {
              yOffset = addText(
                `Program Name: ${cgpaInfo.programName}`,
                15,
                yOffset
              );
            }
