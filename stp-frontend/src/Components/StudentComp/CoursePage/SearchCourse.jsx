...
const SearchCourse = () => {
  ...
  const fetchCountries = async () => {
    try {
      ...
      if (result.data) {
        ...
        const malaysia = result.data.find(country =>
  ...
  const renderPrograms = () => {
    if (!programs.length) {
      return (
        <div className="blankslate-courses text-center" style={{ marginLeft: "100px" }}>
          <img className="blankslate-courses-top-img" src={emptyStateImage} alt="Empty State" />
    ...
    return programs.map((program, index) => (
      <React.Fragment key={program.id}>
        <div className="card mb-4 degree-card" style={{ position: "relative", height: "auto" }}>
              ...
              <Col md={6} lg={6}>
                <div className="card-image mb-3 mb-md-0">
                  <h5 className="card-title">
                    <Link to={`/courseDetails/${program.id}`} style={{ color: "black" }}>
                      {program.name}
                    </Link>
                  </h5>
                        ...
                        <div>
                          <Row style={{ paddingTop: "10px" }}>
                            ...
                            <div style={{ marginTop: "10px" }}>
                              ...
                              <span style={{ paddingLeft: "20px" }}>{program.mode}</span>
                            </div>
                            <div style={{ marginTop: "10px" }}>
  ...
  return (
    <Container>
      ...
      <Row className="align-items-center mb-2 mb-md-0" style={{ marginTop: "10px" }}>
        {/* Country Dropdown */}
        ...
        <Col xs={12} sm={4} md={3} lg={2} className="mb-2 mb-sm-0">
          <ButtonGroup className="w-100">
            <Dropdown as={ButtonGroup} className="w-100">
              ...
              >
                {selectedInstitute ? selectedInstitute.core_metaName : "University"}
              </Dropdown.Toggle>
              <Dropdown.Menu>
      ...
      <Container className="my-5">
        <Row>
          ...
          <Col md={4} className="location-container" style={{ backgroundColor: "white", padding: "20px" }}>
            {/* Desktop Filters */}
            ...
            {/* Mobile Accordion Filters */}
            <Accordion defaultActiveKey="0" className="custom-accordion d-md-none">
              {/* Location Filter */}
              ...
              <Accordion.Item eventKey="2">
                ...
                <Accordion.Body className="custom-accordion-body">
                  <Form.Group>
                    {filterData.qualificationList && filterData.qualificationList.length > 0 ? (
                      filterData.qualificationList.map((qualification, index) => (
                        <Form.Check
                          key={index}
                          type="checkbox"
                          label={qualification.qualification_name}
                          checked={selectedFilters.qualifications?.includes(qualification.id)}
                          onChange={() => handleFilterChange('qualifications', qualification.id)}
                        />
                      ))
                    ) : (
              ...
              <Accordion.Item eventKey="3">
                ...
                <Accordion.Body className="custom-accordion-body">
                  <Form.Group>
                    {filterData.studyModeListing && filterData.studyModeListing.length > 0 ? (
                      filterData.studyModeListing.map((mode, index) => (
                        <Form.Check
                          key={index}
                          type="checkbox"
              ...
              <Accordion.Item eventKey="4">
                ...
                <Accordion.Body className="custom-accordion-body">
                  <Form.Group>
                    {filterData.intakeList && filterData.intakeList.length > 0 ? (
                      filterData.intakeList.map((intake, index) => (
                        <Form.Check
                          key={index}
                          type="checkbox"
