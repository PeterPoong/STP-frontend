...
const ApplicationSummary = ({ formData = fakeFormData }) => {
      ...
      <div className="backgroundimage">
        <div className="application-summary-container">
          ...
          <div className="application-summary-container-inside rounded ">
            <div className="summary-header p-5 border border-bottom">
              ...
              <>
                <div className="summary-content">
                  ...
                  <div className="academic-results m-3 shadow-lg rounded-5 pt-4 d-flex flex-column">
                    <div className="px-4">
                      <p className="text-secondary fw-bold border-bottom border-2 pb-3 m-0">SPM Subjects and Results</p>
                    </div>
                    <div className="results-grid flex-grow-1 px-4 " style={{ maxHeight: '15rem', overflowY: 'auto' }}>
                      {formData.spmResults && formData.spmResults.map((subject, index) => (
                    ...
                    <div className="grade-summary d-flex justify-content-between align-items-stretch border-top">
                      <div className="overall-grade bg-danger text-white w-75 d-flex justify-content-start">
                        <h3 className="align-self-center px-5">Grade: {formData.overallGrade}</h3>
                      </div>
                      ...
                      {formData.coCurricularActivities && formData.coCurricularActivities.map((activity, index) => (
                        <div key={index} className="activity-item d-flex justify-content-between align-items-center">
                          ...
                          <div className="col-3 text-end">
                            <span className={`position ${activity.position.toLowerCase()} py-1 px-2 rounded-pill`} style={{ fontSize: '0.8rem' }}>{activity.position}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="achievements-grid">
                      {formData.achievements && formData.achievements.map((achievement, index) => (
                        <div key={index} className="achievement-item d-flex justify-content-between align-items-center">
                          ...
                          <div className="col-3 text-end">
                            <span className={`position ${achievement.position.toLowerCase().replace(/\s+/g, '-')} py-1 px-2 rounded-pill`} style={{ fontSize: '0.8rem' }}>{achievement.position}</span>
                          </div>
                        </div>
                      ))}
                    </div>
