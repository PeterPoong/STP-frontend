...
const StudentDetailView = ({
  ...
  const [uploadedFrontIc, setUploadedFrontIcFile] = useState(null);
  ...
  useEffect(() => {
    if (applicantDetails) {
      if (applicantDetails.form_status === 2) {
        // Pending
    ...
    try {
      ...
      if (result.success) {
        if (
          selectedCategory === 32 &&
          result.data.spm &&
          Array.isArray(result.data.spm.subject)
        ) {
          setTranscriptSubjects(result.data.spm.subject);
        } else if (
          selectedCategory === 85 &&
          result.data.trial &&
          Array.isArray(result.data.trial.subject)
        ) {
  ...
  const fetchTranscriptDocuments = async () => {
    try {
      ...
      if (result.success) {
        ...
        throw new Error(
          result.message || "Failed to fetch transcript documents"
        );
      }
    } catch (error) {
  ...
  const fetchApplicantDetails = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch applicant details. Status: ${response.status}`
        );
      }
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      const applicationId =
        basicInfo.id || basicInfo.submitted_form_id || basicInfo.application_id;
      if (!applicationId) {
  ...
  const fetchOtherDocuments = async (page = 1) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch other documents. Status: ${response.status}`
        );
      }
      ...
      <div className="academic-results m-3 shadow-lg rounded-5 pt-4 d-flex flex-column">
        <div className="px-4">
          <div className=" d-flex  sas-pointer-div px-0 ">
            ...
            <BsCaretDownFill
              size={30}
              className="sas-pointer align-self-start"
            />
          </div>
        </div>
        ...
        <div className="grade-summary d-flex justify-content-between align-items-stretch border-top">
          ...
          <Button
            ...
            disabled={accountType !== 65 || transcriptDocuments.length === 0}
            style={{
  ...
  const renderRelatedDocumentsContent = () => {
    ...
    const filteredDocuments = documents.filter((doc) => {
      ...
      if (activeDocumentTab === "academic") {
        ...
        fileName = doc.studentMedia_location
        ...
        fileName = doc.achievement_media
  ...
  //   link.href = fileUrl; // URL to the file you want to download
  //   link.download = fileName; // Desired file name for the download
    ...
    <div className="applicant-app-container-applycourse">
      <div className="applicant-backgroundimage">
        <div className="ms-4 mt-3 d-flex mb-0 " onClick={handleBackPage}>
          ...
          <h5 className="ms-2 mt-1" style={{ color: "#ad736c" }}>
            Back
          </h5>
        </div>
          ...
          <div className="application-summary-container-inside">
            <div className="application-summary-container-inside rounded">
              <div className="applicant-summary-header border border-bottom">
                  ...
                  <div>
                    ...
                    <p className="my-0 text-secondary mt-2">
                      ...
                      <span className="text-black ms-2">
                        {applicantDetails.course_name || ""}
                      </span>
                    </p>
                  </div>
                </div>
                <span
                  className={`applicant-status-button ms-auto ${
                    applicantDetails?.form_status === 4
                      ? "accepted"
                      : applicantDetails?.form_status === 3
                      ? "rejected"
                      : "pending"
                  } text-white`}
                >
                ...
                <Button
                  className="applicant-chat-button align-items-center justify-content-center text-nowrap"
                  onClick={handleWhatsAppClick}
                >
                  <BsWhatsapp
                    className="me-2 whatsapp-button text-nonwrap"
                    size={20}
                  />
                  Chat on WhatsApp
                </Button>
              ...
              <div className="summary-tabs d-flex flex-wrap px-2 py-2 justify-content-center">
                ...
                <Button
                  variant="link"
                  className={`tab-button ${
                    activeTab === "accept-reject" ? "active" : ""
                  }`}
                ...
                <div className="summary-content">
                    ...
                    <div className="info-grid">
                      <div className="row">
                        ...
                        <div className="col-md-6 mb-3">
                          ...
                          <p className="d-flex align-items-center">
                            <span className="me-2">
                              {`${basicInfo?.student_countryCode || ""} ${
                                basicInfo?.student_contactNo || ""
                              }`}
                            </span>
                            <span
                              className="copy-icon-wrapper"
                                ...
                                achievements.map((achievement, index) => (
                                  ...
                                  >
                                    <div className="col-12 col-sm-4">
                                      ...
                                      <p className="mb-0 name-restrict text-muted">
                                    ...
                                    <div className="col-6 col-sm-5 mx-auto text-end">
                                      <span
                                        className={`position ${(
                                          achievement.title?.core_metaName?.toLowerCase() ??
                                          ""
                                        ).replace(
                                          /\s+/g,
                                          "-"
                                      ...
                                      >
                                        {achievement.title?.core_metaName ||
                                          "No Title"}
                                      </span>
                                    </div>
                                  </div>
                    ...
                    <>
                      <div className="sdv-cocurriculum m-3 shadow-lg p-4 rounded-5 d-flex align-items-center justify-content-center flex-column ">
                        ...
                        <p className="text-center text-white mt-3 px-5">
                          This feature is locked and available only with a
                          premium account.
                        </p>
                        <div className="sdv-div-plan-button rounded-pill mt-3">
                          <button
                            className="plan-button rounded-pill"
                    ...
                    <div className="col-12">
                      <div className="achievements m-3 shadow-lg p-4 rounded-5">
                        ...
                        {uploadedFrontIcFileName ? (
                            ...
                            <Form.Group controlId="photoUpload">
                              ...
                              >
                                ...
                                >
                                  {uploadedFrontIcFileName ? (
                                    <a
                                      href={uploadedFrontIcFileUrl} // Use the file object if URL is empty
                                      target="_blank"
                                      style={{
                                        color: "#B71A18",
                                        fontSize: "13px",
                                ...
                                >
                                  <a
                                    ...
                                    href={uploadedFrontIcFileUrl}
                                    download={uploadedFrontIcFileName}
                                    style={{
                                      color: "#B71A18",
                                      fontSize: "11px",
                                      padding:
                          ...
                          <p className="text-center text-muted">
                            No passport front ic uploaded yet.
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="co-curriculum m-3 shadow-lg p-4 rounded-5">
                    ...
                    {uploadedBackIcFileName ? (
                        ...
                        <Form.Group controlId="photoUpload">
                          ...
                          >
                            ...
                            >
                              {uploadedBackIcFileName ? (
                                <a
                                  href={uploadedBackIcFileUrl} // Use the file object if URL is empty
                                  target="_blank"
                                  style={{
                                    color: "#B71A18",
                                    fontSize: "13px",
                            ...
                            >
                              <a
                                // href="http://192.168.0.75:8000/storage/studentDocument/frontIc1745200884.png"
                                href={uploadedBackIcFileUrl}
                                download={uploadedBackIcFileName}
                                style={{
                                  color: "#B71A18",
                                  fontSize: "11px",
                                  padding:
                      ...
                      <p className="text-center text-muted">
                        No passport back ic uploaded yet.
                      </p>
                    )}
                  </div>

                  {/* passport */}
                  <div className="m-3 shadow-lg p-4 rounded-5">
                    <p className="text-secondary fw-bold border-bottom border-2 pb-3">
                      Passport
                    </p>
